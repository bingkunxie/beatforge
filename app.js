...
(() => {
  ...
  function save() {
    localStorage.setItem(KEY, JSON.stringify({
      ...
      master: state.master, kit: state.kit, patterns: state.patterns,
    }));
    flash($("#save"), "Saved ✓");
  }
  function load() {
    const raw = localStorage.getItem(KEY);
    if (!raw) { flash($("#load"), "Nothing"); return; }
    try {
      ...
      audio(); masterGain.gain.value = state.master;
      setKit(state.kit);
      buildBank(); buildGrid();
      flash($("#load"), "Loaded ✓");
    } catch { flash($("#load"), "Error"); }
  }
  ...
  $("#bpm").oninput = e => { state.bpm = Math.min(240, Math.max(40, +e.target.value || 120)); };
  ...
  $("#steps").onchange = e => { state.steps = +e.target.value; currentStep = 0; buildGrid(); };
  $("#clear").onclick = () => {
  ...
  document.addEventListener("keydown", e => {
    ...
    if (e.key >= "1" && e.key <= "4") { state.current = +e.key - 1; buildBank(); buildGrid(); }
  });
